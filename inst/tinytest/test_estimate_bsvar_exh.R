...
expect_equal(
  ...
  info = "estimate_bsvar_exh: regime draws use compact zero-based storage."
)
expect_true(
  all(run_no1$posterior$xi == 0),
  info = "estimate_bsvar_exh: compact regime draws contain zero-based indices."
)

set.seed(1)
suppressMessages(
  specification_no2 <- specify_bsvar_exh$new(us_fiscal_lsuw)
)
...
expect_identical(
  ...
  info = "estimate_bsvar_exh: the last_draw(s) of two runs to be identical."
)

expect_identical(
  run_no1$posterior$B[1,1,1],
  run_no2$posterior$B[1,1,1],
  info = "estimate_bsvar_exh: the first draws of two runs to be identical."
)

expect_identical(
  run_no1$last_draw$starting_values$B[1,1],
  run_no3$last_draw$starting_values$B[1,1],
  info = "estimate_bsvar_exh: the last_draw(s) of a normal and pipe run to be identical."
)
...
A[1, 3] = FALSE
suppressMessages(
  specification_no4 <- specify_bsvar_exh$new(us_fiscal_lsuw, A = A)
)
run_no4             <- estimate(specification_no4, 3, 1, show_progress = FALSE)
expect_true(
  all(run_no4$posterior$A[1, 3, ] == 0),
  info = "estimate_bsvar_exh: the A matrix is restricted correctly."
)
