...
expect_equal(
  ...
  info = "estimate_bsvar_mix: regime draws use compact zero-based storage."
)
expect_true(
  all(run_no1$posterior$xi %in% 0:1),
  info = "estimate_bsvar_mix: compact regime draws contain valid zero-based indices."
)
...
A[1, 3] = FALSE
suppressMessages(
  specification_no4 <- specify_bsvar_mix$new(us_fiscal_lsuw, M = 2, A = A)
)
run_no4             <- estimate(specification_no4, 3, 1, show_progress = FALSE)
expect_true(
  all(run_no4$posterior$A[1, 3, ] == 0),
  info = "estimate_bsvar_mix: the A matrix is restricted correctly."
)
