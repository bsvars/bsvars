...
expect_identical(
  ...
  info = "estimate_bsvar: the last_draw(s) of a normal and pipe run to be identical."
)
...
A[1, 3] = FALSE
suppressMessages(
  specification_no4 <- specify_bsvar$new(us_fiscal_lsuw, A = A)
)
run_no4             <- estimate(specification_no4, 3, 1, show_progress = FALSE)
expect_true(
  all(run_no4$posterior$A[1, 3, ] == 0),
  info = "estimate_bsvar: the A matrix is restricted correctly."
)
