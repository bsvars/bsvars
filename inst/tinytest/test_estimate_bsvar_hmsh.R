...
set.seed(1)
suppressMessages(
  specification_no2 <- specify_bsvar_hmsh$new(us_fiscal_lsuw, M = 2)
)
...
expect_identical(
  ...
  info = "estimate_bsvar_hmsh: the last_draw(s) of two runs to be identical."
)

expect_identical(
  run_no1$posterior$B[1,1,1],
  run_no2$posterior$B[1,1,1],
  info = "estimate_bsvar_hmsh: the first draws of two runs to be identical."
)

expect_identical(
  run_no1$last_draw$starting_values$B[1,1],
  run_no3$last_draw$starting_values$B[1,1],
  info = "estimate_bsvar_hmsh: the last_draw(s) of a normal and pipe run to be identical."
)
...
A[1, 3] = FALSE
suppressMessages(
  specification_no4 <- specify_bsvar_hmsh$new(us_fiscal_lsuw, M = 2, A = A)
)
run_no4             <- estimate(specification_no4, 3, 1, show_progress = FALSE)
expect_true(
  all(run_no4$posterior$A[1, 3, ] == 0),
  info = "estimate_bsvar_hmsh: the A matrix is restricted correctly."
)
