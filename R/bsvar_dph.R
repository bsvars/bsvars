...
#' an infinite mixture of normal components via Gibbs sampler
#'
#' @description Estimates the SVAR with non-normal residuals following an infinite mixture of normal distributions proposed by Woźniak & Droumaguet (2022).
...
#' 
#' Finally, the structural shocks, \code{U}, are temporally and contemporaneously independent and finite-mixture of normals distributed with zero mean.
...
#' where \code{s_t} is a the regime indicator of 
...
#' 
#' NOTE: The estimation of the latent regime process for this model presumes a large number of components of the finite mixture model
#' and the components are allowed to be doubled by other estimated components or have zero number of occurrences at any MCMC iteration.
#' 
...
#'  \item{PR_TR}{an \code{MxM} matrix  of staring values for the transition matrix. These have to be non-negative values summing to 1 by rows. All rows have to be the same.}
...
bsvar_dph <- function(S, Y, X, prior, VB, starting_values) {
  .Call(`_bsvars_bsvar_msh_cpp`, S, Y, X, prior, VB, starting_values, FALSE, FALSE)
}
