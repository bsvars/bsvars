...
#'
#' @description Estimates the SVAR with exogenous heteroskedastic regime changes 
#' with \code{M} regimes (MS(M)) proposed by Woźniak & Droumaguet (2022).
...
#' \deqn{Y = AX + E}
#' where \eqn{Y} is an \code{NxT} matrix of dependent variables, \eqn{X} is a \code{KxT} 
#' matrix of explanatory variables, \eqn{E} is an \code{NxT} matrix of reduced form 
#' error terms, and \eqn{A} is an \code{NxK} matrix of autoregressive slope coefficients 
#' and parameters on deterministic terms in \code{X}.
#' 
...
#' where \eqn{s_t} is an exogenous process driving the time-variability of 
...
#' The model selection also with this respect is made using function \code{\link{specify_bsvar_exh}}.
#' 
#' @param specification an object of class BSVAREXH generated using the \code{specify_bsvar_exh$new()} function.
...
#' 
#' @return An object of class PosteriorBSVAREXH containing the Bayesian estimation 
...
#' }
#' 
#' \code{last_draw} an object of class BSVAREXH with the last draw of the current 
#' MCMC run as the starting value to be passed to the continuation of the MCMC estimation using \code{estimate()}. 
#' 
#' @seealso \code{\link{specify_bsvar_exh}}, \code{\link{specify_posterior_bsvar_exh}}, \code{\link{normalise_posterior}}
#'
...
estimate.BSVAREXH <- function(specification, S, thin = 1, show_progress = TRUE) {
  ...
  # estimation
  qqq                 = .Call(`_bsvars_bsvar_exh_cpp`, S, data_matrices$Y, data_matrices$X, prior, VB, VA, starting_values, normal, thin, show_progress)
  
  specification$starting_values$set_starting_values(qqq$last_draw)
  output              = specify_posterior_bsvar_exh$new(specification, qqq$posterior)
...
#' @inherit estimate.BSVAREXH
#' 
#' @method estimate PosteriorBSVAREXH
#' 
#' @param specification an object of class PosteriorBSVAREXH generated using the \code{estimate()} function.
...
estimate.PosteriorBSVAREXH <- function(specification, S, thin = 1, show_progress = TRUE) {
  ...
  output              = specify_posterior_bsvar_exh$new(specification$last_draw, qqq$posterior)
